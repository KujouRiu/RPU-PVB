...
from circle import OnlineTripletLoss, pdist_js, pdist_l2
from dconv import *
...
def train():
    ...
    ssd_net = build_robust_ssd('train', cfg['min_dim'], cfg['num_classes'], amp=args.amp and args.multi_gpu,
                               CFR=args.cfr, CFR_layer=args.cfr_layer, multi_fc=args.multi_fc, K=args.k_count,
    ...
    data_loader = data.DataLoader(dataset, args.batch_size // 2,
                                  ...
                                  pin_memory=False, drop_last=True, generator=torch.Generator(device='cuda'))
    ...
    # pgd = GLH(net, img_transform=(lambda x: x - dataset_mean_t, lambda x: x + dataset_mean_t))
    # pgd.set_para(eps=args.step_size, alpha=lambda:args.step_size, iters=args.atk_iters)
    pgd.set_para(eps=args.step_size, alpha=lambda: args.step_size, iters=6)
    ...
    if args.adc > 0:
        adv_weights = torch.FloatTensor(
            [1] * (data_loader_adv.batch_size + data_loader.batch_size) + [data_loader_adv.batch_size / args.adc] * (
                args.adc))
    ...
    for iteration in range(args.start_iter // pgd.iters, args.max_iter // pgd.iters):
        ...
        for im_clean, at_img, at_targets in adv_generator:
            vutils.save_image(im_clean, './test.jpg', normalize=True)
            vutils.save_image(at_img, './at.jpg', normalize=True)
            ...
            with amp.autocast(enabled=args.amp):
                ...
                # backprop
                loss_l, loss_c = criterion_mlb(out,
                                               targets + at_targets + targets_sample if args.adc > 0 else targets + at_targets,
                                               weights=adv_weights if args.adc > 0 else None)
                # loss_control_factor
                '''                             
                if inter_iteration == 0:
                    loss_control_factor = 3                                       
                loss = (loss_l + loss_c + loss_tri * loss_control_factor) * 0.75
                if inter_iteration % 5000 == 0:
                    loss_control_factor = float(10-loss)
                '''
                ...
                if args.cfr:
                    ...
                    # cosloss
                    '''
                    recons_loss = 1 - torch.mean(torch.cosine_similarity(torch.cat((images, im_clean, img_sample), dim=0),
                                             torch.cat((recons1, recons2), dim=0)))
                    '''
                    #
                    ...
                    '''
                    ma=50
                    mask = torch.ones_like(fft.fft(fft.fftshift(fft.rfft(torch.cat((images, im_clean, img_sample), dim=0)))))
                    mask[: , : , 75 - ma :75 + ma, 75 - ma:75 + ma] = 0 

                    ori_img_fft = fft.irfft(fft.ifftshift(mask * fft.fftshift(fft.rfft(torch.cat((images, im_clean, img_sample), dim=0)))))
                    attack_img_fft = fft.irfft(fft.ifftshift(mask * fft.fftshift(fft.rfft(torch.cat((recons1, recons2), dim=0)))))



                    recons_loss = F.mse_loss(ori_img_fft,
                                             attack_img_fft)
                    ...
                    '''
                    ma=50
                    mask = torch.zeros_like(fft.fft(fft.fftshift(fft.rfft(torch.cat((images, im_clean, img_sample), dim=0)))))
                    mask[: , : , 75 - ma :75 + ma, 75 - ma:75 + ma] = 1 
                    ...
                    kld_loss = torch.mean(
                        -0.5 * torch.mean((1 + logvar - mu ** 2 - logvar.exp()).view(images.size(0), -1), dim=1), dim=0)

                    # mseloss
                    loss = loss + (recons_loss * 0.16 + kld_loss * 5) / 255.0

                    # fft loss
                    # loss = loss + (recons_loss * 1.6 + kld_loss * 5) / 255.0

                    # cosloss
                    # loss = loss + 50 * recons_loss  + (kld_loss * 5) / 255.0
            ...
            if inter_iteration % 50 == 0:
                t1 = time.time()
                if args.cfr:
                    logger.info('iter ' + repr(
                        inter_iteration) + ' || Loss: %.4f || recons_loss: %.4f || kld_loss: %.4f || timer: %.4f sec.' %
            ...
            if inter_iteration != 0 and inter_iteration % 50000 == 0:
                logger.info(f'Saving state, iter: {inter_iteration}')
                torch.save(ssd_net.state_dict(),
                           os.path.join(args.save_folder, f'ssd300_{task_name}_{inter_iteration}_0.25.pth'))

            inter_iteration += 1

    torch.save(ssd_net.state_dict(),
               os.path.join(args.save_folder, f'ssd300_{task_name}_final_{args.max_iter}_0.25.pth'))
